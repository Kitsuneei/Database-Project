...
def top_k_with_histogram(inputs, k, bucket_size=100, memory_size = 1000):
    ...
    while inputs: 
        ...
        if len(run_gen) == memory_size:
            ...
            Tracker.append([run_cnt, prior_input_size + len(run_gen), prior_key, secondary_cnt] + bucket_quartile)
            run_gen = []
    ...
    if len(run_gen) > 0:
        ...
        secondary_cnt += len(run_gen)
        run_cnt += 1
        Tracker.append([run_cnt, len(run_gen), cutoff_key, secondary_cnt]+bucket_quartile)
...
df_result.to_csv("Table_"+str(Experiment)+".csv")
