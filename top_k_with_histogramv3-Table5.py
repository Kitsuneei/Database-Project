...
def top_k_with_histogram(inputs, k, bucket_size=100, memory_size = 1000):
    ...
    while inputs: 
        ...
        if len(run_gen) == memory_size:
            ...
            bucket_key = [(median(bucket), len(bucket))for bucket in bucket_list]
            ...
            for i in bucket_key:
                ...
                if histogram_data_total < k:
                    ...
                    histogram_data_total -= discard_bucket[1] / 2
                    heappush_max(histogram_queue, i)
                    
                histogram_data_total += i[1] / 2
...
bucket_size = 1000
Result_list = []
for input_size in [6000,7000,10000,20000,50000,100000,200000,500000,1000000,2000000,5000000,10000000,20000000,50000000,100000000]:
    random.seed(10)
    inputs = generate_data(input_size)
    Result = top_k_with_histogram(inputs, k, bucket_size, memory_size)
    #print(Result[2])
    Result_list.append([input_size] + Result[1][-1])
                             
                             
                             
df_result = pd.DataFrame(Result_list, columns = ['InputSize', 'RunNum', 'Remaining_Inputs', 'CutOff', 'Secondary_Cnt'])
